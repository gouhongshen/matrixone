	...
	eng.dynamicCtx.once.Do(func() {
		...
		go func() {
			...
			for {
				...
				if eng.dynamicCtx.initCronTask(taskCtx) {
					break
				}
			}
		}()
...
func (d *dynamicCtx) alphaTask(
	...
	defer func() {
		...
		logutil.Info(logHeader,
			...
			zap.Int("request cnt", requestCnt),
...
func (d *dynamicCtx) getChangedTableList(
	ctx context.Context,
	service string,
	eng engine.Engine,
	accs []uint64,
	...
	*to = types.TimestampToTS(*resp.Newest)
	...
	for i := range resp.AccIds {
		tp, ok := buildTablePairFromCache(de,
			resp.AccIds[i], resp.DatabaseIds[i], resp.TableIds[i], *to, false)
		if !ok {
			continue
		}
