...
const (
	...
	// alter command for TN. Update batches for mo_tables and mo_columns will fall into the category of INSERT and DELETE.
	WS_ALTER
)

func IsWSData(t int) bool {
	return t == WS_DATA_ROWS || t == WS_DATA_OBJS
}

func IsWSTombstone(t int) bool {
	return t == WS_TOMBSTONE_ROWS || t == WS_TOMBSTONE_OBJS
}

func IsWSInMemRows(t int) bool {
	return t == WS_DATA_ROWS || t == WS_TOMBSTONE_ROWS
}

func IsWSPersistedObjs(t int) bool {
	return t == WS_DATA_OBJS || t == WS_TOMBSTONE_OBJS
}
...
var (
	typesNames = map[int]string{
		WS_DATA_ROWS:      "data rows",
		WS_DATA_OBJS:      "data objects",
		WS_ALTER:          "alter",
		WS_TOMBSTONE_ROWS: "tombstone rows",
		WS_TOMBSTONE_OBJS: "tombstone objects",
	}
)
...
func (txn *Transaction) adjustUpdateOrderLocked(writeOffset uint64) error {
	if txn.statementID > 0 {
		...
		for i := writeOffset; i < uint64(len(txn.writes)); i++ {
			if !txn.writes[i].isCatalog() && IsWSTombstone(txn.writes[i].typ) {
...
func (txn *Transaction) gcObjs(start int) error {
	...
	for i := start; i < len(txn.writes); i++ {
		...
		if txn.writes[i].typ == WS_DATA_OBJS {
		...
		} else if txn.writes[i].typ == WS_TOMBSTONE_OBJS {
...
func (e *Entry) String() string {
	...
	return fmt.Sprintf("Entry{type:%v, note:%v, table:%v, db:%v, account:%v, tableId:%v, dbId:%v, bat:%v}",
		typesNames[e.typ], e.note, e.tableName, e.databaseName, e.accountId, e.tableId, e.databaseId, batinfo)
}
