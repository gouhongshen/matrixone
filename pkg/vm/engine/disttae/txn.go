...
func (txn *Transaction) dumpBatchLocked(offset int) error {
	...
	for i := offset; i < len(txn.writes); i++ {
		...
		if txn.writes[i].typ == INSERT && txn.writes[i].bat.Attrs[0] != catalog.ObjectMeta_ObjectStats {
	...
	for key := range mp {
		...
		statsBat := s3Writer.GetObjectStatsBat()
		statsBat.Vecs = statsBat.Vecs[1:]
		statsBat.Attrs = statsBat.Attrs[1:]
		statsBat.SetRowCount(statsBat.Vecs[0].Length())
...
func (txn *Transaction) insertPosForCNBlock(
	...
	dbName string,
	tbName string) error {
	stats := objectio.ObjectStats(vector.MustBytesCol(vec)[0])
	blks := catalog.UnFoldBlksFromObjStats(&stats)
...
func (txn *Transaction) WriteFileLocked(
	...
	if sid, err := types.ParseUuid(strings.Split(fileName, "_")[0]); err != nil {
		statsBytes := vector.MustBytesCol(bat.GetVector(0))
		for idx := 0; idx < len(statsBytes); idx++ {
			stats := objectio.ObjectStats(statsBytes[idx])
			if !stats.IsZero() {
...
func (txn *Transaction) mergeCompactionLocked() error {
	...
	for key, blks := range compactedBlks {
		...
		tbl := rel.(*txnTable)
		objectStats, err := tbl.mergeCompaction(blks)
		if err != nil {
			return err
		}
		if len(objectStats) > 0 {
			bat := batch.NewWithSize(1)
			bat.Attrs = []string{catalog.ObjectMeta_ObjectStats}
			bat.SetVector(0, vector.NewVec(types.T_text.ToType()))
			for _, stats := range objectStats {
				if stats.IsZero() {
					continue
				}
				vector.AppendBytes(
					bat.GetVector(0), stats.Marshal(),
					false, tbl.db.txn.proc.GetMPool())
			}
